...
It also convert the cryptocurrenciy values from Satoshi to approximate USD prices.

The only parameter of importance is "period" the frequency of your data.
...
#67 coins on poloniex available (as of 18.4.2018)

#update -5.08.2018 RIC was taken off the exchange
##RIC,XVC,PINK,BELA,BCY,FLO ,BLK,NXC,RADS,FLDC COIN DOESNT WORK, making it 57
...
#Irrelevant outside of Jupyter

##86400 #7200
#if period >= 14400:
#    df_scatter(combined_df,'log Cryptoprices (USD)', seperate_y_axis=False,y_axis_label='Coin Value (USD)',scale='log')
#    df_scatter(combined_df,'Cryptoprices (USD)', seperate_y_axis=False,y_axis_label='Coin Value (USD)',scale='linear')
#    df_scatter(satoshi_df,'Cryptoprices (Satoshi)', seperate_y_axis=False,y_axis_label='Coin Value (Satoshi)',scale='linear')
#else:
##Do this for higher frequency data
#    #satoshi_df_2018 = satoshi_df[satoshi_df.index.year == 2018]
#    df_scatter(satoshi_df,'Cryptoprices (Satoshi)', seperate_y_axis=False,y_axis_label='Coin Value (Satoshi)',scale='linear')
...
#Due to a potentially spurios covariance matrix because of non-stationary time series first differences should be taken:
#.diff()
...
#if you want so save files (e.g. when only using console)
#fig = combined_df.plot()
